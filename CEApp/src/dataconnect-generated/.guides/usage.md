...
```ts
import { useAddNewService, useListContractorProfiles, useUpdateProjectStatus, useGetMyReviews } from '@dataconnect/generated/react';
// The types of these hooks are available in react/index.d.ts

const { data, isPending, isSuccess, isError, error } = useAddNewService(addNewServiceVars);

const { data, isPending, isSuccess, isError, error } = useListContractorProfiles();

const { data, isPending, isSuccess, isError, error } = useUpdateProjectStatus(updateProjectStatusVars);

const { data, isPending, isSuccess, isError, error } = useGetMyReviews(getMyReviewsVars);
